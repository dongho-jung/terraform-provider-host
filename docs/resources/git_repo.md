...
- `url` (String) Git remote URL, such as an HTTPS URL, SSH URL, or local path accepted by `git clone`. Changes update the existing remote in place after verifying that it still matches Terraform state.
